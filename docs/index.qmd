...
* Reference distribution often chosen as a standard Gaussian: $\pi_{\text{ref}} = \normal(0,I)$.

* To sample $\blue{X \sim q_\lambda}(dx)$: generate $\green{Z \sim \pi_{\text{ref}}(dz)}$ and apply $T_\lambda$:

$$
\begin{aligned}
\blue{X} \; &= \; T_\lambda(\green{Z}) \qquad \textrm{where} \qquad \green{Z \sim \pi_{\text{ref}}}\\
\blue{q_{\lambda}} \; &= \; (T_{\lambda} )_{\sharp} ( \green{\pi_{\text{ref}}} )
\end{aligned}
$$

* For $\blue{x} = T_\lambda(\green{z})$, the density $q_\lambda$ reads:
...
#### (incomplete) history of normalizing flows

::: {style="text-align:center;"}
![](./data/density_estimation_eric.jpg){width=1000}
:::

::: {style="text-align:center;"}
![](./data/variational_inference_rezende.jpg){width=1000}
:::
...
* Can be used to approximate an unormalized distribution:
...
::: {style="text-align:center;"}
![](./data/neural_ODE.jpg){height=200}
:::

* Normalizing flow: $Z \to Z_1 \to Z_2 \to \ldots \to Z_K \to X$

* What about the continuous limit: ["Neural Ordinary Differential Equation"](https://arxiv.org/abs/1806.07366)

$$
\frac{dX}{dt} = \blue{\mu_{\lambda}(t,X)} \qquad \textrm{with} \qquad
X_0 \sim \pi_{\text{ref}}(dx)
$$

::: {style="text-align:center;"}
![](./data/neural_ODE_field.jpg){height=250}
:::
...
* Can be trained with Maximum Likelihood Estimation (MLE)

$$
\P( X_T \in dx \, | \, X_0 = x_0) \; = \; \textrm{(some formula discussed later)}
$$

* **Issue**: this typically requires  <span style='color:blue'>solving the ODE</span> at each iteration

* This is too <span style='color:blue'>slow</span> for large datasets, although the idea is very appealing!

::: {style="text-align:center;"}
![](./data/neural_ODE_results.jpg){height=250}
:::
...
* Neural ODEs are too slow to train because require solving ODEs
* Instead, design an appropriate drift function $\mu(t,x)$ so that the diffusion
$$dX = \blue{\mu_{\lambda}(t,X)} \, dt + \sigma \, dW$$
...
::: {style="text-align:center;"}
![](./data/deep_unsupervised.jpg){width=800}
:::

::: {style="text-align:center;"}
![](./data/deep_unsupervised_examples.jpg){width=800}
:::
...
$$
X_{t+s} | (X_t=x_t) \; \stackrel{\mathcal{L}}{=} \;\alpha_s x_t + \sqrt{1-\alpha_s^2} \, \bfn \qquad \text{with} \qquad
\alpha_s = \exp(-s/2)
$$
for isotropic Gaussian noise term $\bfn \sim \pi_{\ref} = \normal(0,I)$. 

* <span style='color:blue'>**Crucial:**</span> $X_t$ can be simulated without Euler scheme!

* The marginal distribution at time $0 \leq t \leq T$ is denoted as

$$
\blue{ X_t \sim p_t(dx) \; = \; \int \P(X_t \in dx \mid X_0 = x_0) \, \pi_{\data}(dx_0) }
$$
...
3. Algebra gives the reverse dynamics!
...
* Noising OU process: $\blue{dX_t = - \frac12 X_t \, dt + dW}$ with $X_0 \sim \pi_{\data}(dx)$
...
* A number of ML techniques to evaluate the score $\red{\cS(t,x)}$ from data, i.e. how to estimate the gradient of the density if we have samples from this density. We will concentrate on "denoising score matching"
...
* Several attempts to scale this approach: sometimes works very well
...
* Note that generating samples from $p_t(dx)$ is straightforward and does not require solving ODEs or SDEs: it is <span style='color:blue'>**simulation-free**</span>.

$$
X_t = \alpha_t \, X_0 + \sqrt{1-\alpha_t^2} \, \bfn
$$
...
$$
\begin{align}
\P(X_t \in dx \, | \, X_0 = x_0) &= \frac{1}{\cZ_t} \int \exp\curBK{-\frac{(x - \alpha_t \, x_0)^2}{2 (1-\alpha_t^2)}} \, \pi_{\data}(d x_0)\\
\cS(t,x)  &= \nabla \log \int  \exp\curBK{-\frac{(x - \alpha_t \, x_0)^2}{2 (1-\alpha_t^2)}}
\; \pi_{\data}(d x_0)
\end{align}
$$
...
$$
\cS(t,x) \; = \; - \frac{ \int \blue{ \frac{(x - \alpha_t \, x_0)}{(1-\alpha_t^2)} } \, p(X_t=x, X_0=x_0) \, dx_0}{\int  p(X_t=x, X_0=x_0) \, dx_0}
$$
* One can recognize a <span style='color:blue'>**conditional expectation**</span>:

$$
\cS(t,x) \; = \; -\E\sqBK{  \blue{ \frac{(x - \alpha_t \, X_0)}{(1-\alpha_t^2)}} \mid X_t=x}
$$
...
* Consider $(X,Y) \in \bbR^{d_X} \times \bbR^{d_Y}$: the conditional expectation $F_\star(x) = \E[Y \mid X=x]$ is the function $F_\star: \bbR^{d_X} \to \bbR^{d_Y}$ that minimizes

$$
F \; \mapsto \; \E\sqBK{ \left\| Y - F(X) \right\|^2}
$$

* To approximate the conditional expectation function, parametrize with a neural network and minimize with stochastic gradient descent:

$$
\theta \; \mapsto \; \E\sqBK{ \left\| Y - F_\theta(X) \right\|^2} \approx \frac{1}{N} \sum_{i=1}^N \left\| y_i - F_\theta(x_i) \right\|^2
$$

::: {style="text-align:center;"}
![](./data/neural_net.png){height=200}
:::
...
* The score can be expressed as:
$$
\cS(t,x) \; = \; \nabla_x \log p_t(x) \; = \; -\frac{x - \alpha_t \, \red{\widehat{x}_0(t,x)} }{1-\alpha_t^2}
$$
where $\red{\widehat{x}_0(t,x)}$ is an estimate of $X_0$ given $X_t=x$:
$$
...
* <span style='color:blue'>**Important take away:**</span> one can build a sampler out of a denoiser!

---

* Building a sampler out of a denoiser: if one knows how to approximately compute expectation with respect to a target density $\pi(dx)$, one can generate approximate samples from $\pi(dx)$

* Related to the idea of <span style='color:blue'>"stochastic localization"</span> that is actively investigated by several communities nowadays

::: {style="text-align:center;"}
![](./data/stoch_loc.png){width=800}
:::
...
* Parametrize $\widehat{x}_0: [0,T] \times \bbR^d \to \bbR^d$ with a neural-net and minimize

$$
\cL( \theta ) \equiv \E \curBK{ \int_{t=0}^T \, \lambda(t) \, \left\| X_0 - \widehat{x}_0(\tau, X_{t}) \right\|^2 \, dt}
$$
...
$$
\cS(t,x) \; = \; -\frac{x - \alpha_t \, \widehat{x}_0(t,x)}{1-\alpha^2_t}
\; = \; 
\nabla \log p_t(x)
$$
...
* Common implementation: use a neural network to estimate the noise instead!

$$
\begin{align}
x
&= \E[ \alpha_t \, X_0 + \sqrt{1-\alpha_t^2} \, \bfn \mid X_t = x]\\
&= \alpha_t \, \blue{\widehat{x}_0(t,x)} + \sqrt{1-\alpha_t^2} \, \green{\widehat{\bfn}(t,x)}
\end{align}
$$

where $\widehat{\bfn}(t,x) = \E[\bfn \mid X_t = x]$.

* Estimating $X_0$ or the noise term $\bfn$ is equivalent

$$
\begin{align}
\cS(t,x) 
&= -\frac{x - \alpha_t \, \blue{\widehat{x}_0(t,x)}}{1-\alpha^2_t}
= -\frac{\green{\widehat{\bfn}(t,x)}}{\sqrt{1-\sigma_t^2}}
\end{align}
$$
...
* Denoiser: $\blue{ \widehat{x}_0: [0,T] \times \bbR^d \to \bbR^d }$

* Can be useful to include "Fourier features"

* For images: convolutional layers, U-net, etc..

* For molecules: graph neural networks, etc...

* Physics: include physical constraints, specialized architectures

::: {style="text-align:center;"}
![](./data/unet.jpg){height=250}
:::
...
* Differential Equation $\blue{dX^{\textrm{ODE}}_t = \vec{F}(t,X^{\textrm{ODE}}_t) \, dt}$ with marginal $X^{\textrm{ODE}}_t \sim p^{\textrm{ODE}}_t$

$$
\partial_t p^{\textrm{ODE}}_t = -\nabla \cdot ( p^{\textrm{ODE}}_t \, \vec{F}).
$$

* Can one find $\vec{F}(t,x)$ such that $\red{p_t(x) = p_t^{\text{ODE}}(x)}$ for all times?


* Yes: It suffices to take:

$$
\red{\vec{F}(t,x) = \mu(t,x)  - \frac12 \, \nabla_x \log p_t(x)}
$$
...
$$dX^{\rev}_s = \blue{\frac12 \, X^{\rev}_s \, ds \; + \nabla \log p_{T-s}(X^{\rev}_s) \, ds} \; + \;  \green{dB}$$

* Reverse ODE:

$$dX^{\rev}_s = \blue{\frac12 \, X^{\rev}_s \, ds \; + \red{\frac12} \, \nabla \log p_{T-s}(X^{\rev}_s) \, ds }$$
...
::: {style="text-align:center;"}
![](./data/interpolation.jpg){width=800}

[Paper (2021): "Score-Based Generative Modeling through Stochastic Differential Equations"](https://arxiv.org/abs/2011.13456)
:::
...
\log p_T(x_T)
\; = \;
\log p_0(x_0) \; - \; \int_0^T \blue{\nabla \cdot \vec{F}(t,x_t)} \, dt
$$
...
* Estimatation of the trace of $M \in \mathbb{R}^{D,D}$ for $D \gg 1$
* Unbiased estimate of the trace:

$$
\tr(M) \; = \; \E \curBK{ \bra{\epsilon, M \, \epsilon}}
$$

where $\epsilon$ is a random vector with $\cov(\epsilon) = \mathbf{1} \in \mathbb{R}^{D,D}$

* Estimator of the divergence $\nabla \cdot F = \tr( J_F)$ with jacobian vector products

$$
\blue{
\nabla \cdot \vec{F}(x) \; \approx \;
\frac{1}{N} \sum_{i=1}^N \bra{\epsilon_i, J_F(x) \, \epsilon_i}
}
$$

where $\epsilon_i$ are iid samples from $\normal(0,I)$

---

* What neural architecture for the denoiser?
* SDE or ODE formulation? Mix of both?
* Noise estimation or $X_0$ estimation?
* How to discretize the SDE/ODE?
* Weighting function $\lambda(t)$?

::: {style="text-align:center;"}
![](./data/design_space.png){width=800}

[NeurIPS 2022 Paper Award](https://arxiv.org/abs/2206.00364)
:::
---

#### Stochastic Interpolant & Flow matching

::: {style="text-align:center;"}
![](./data/stoch_interpolant_paper.png){width=800}
:::

::: {style="text-align:center;"}
![](./data/flow_matching.png){width=800}
:::

---

* Denoising diffusion: simulation-free method for fitting a diffusion model to htat transforms Gaussian noise into data

* <span style='color:blue'>Stochastic Interpolant:</span> general simulation-free method for fitting a ODE and SDE that transforms an <span style='color:blue'>arbitrary distribution</span> into data.


::: {style="text-align:center;"}
![](./data/stoch_interpolant.jpg){width=800}

SDE and ODE interpolantions from non-Gaussian data
:::

---

::: {style="text-align:center;"}
![](./data/stoch_interpolant_non_gaussian.jpg){width=800}

Mapping incomplete images to complete images

Source: ["Stochastic Interpolant with data-dependent couplings"](https://arxiv.org/abs/2310.03725)
:::

---

* consider two probability distribution $\blue{\pi_0(dx)}$ and $\green{\pi_1(dx)}$
  * distribution $\blue{\pi_0(dx)}$ is known and easy to sample from
  * distribution $\green{\pi_1(dx) \equiv \pi_{\data}(dx)}$ is of interest

* Assume that one can generate the pair of random variable $(X_0, \red{C})$ where
   * $X_0 \sim \blue{\pi_0(dx)}$
   * quantity $\red{C}$ is "conditionning" random variable
   * the random variable $X_0$ and $C$ do **not** need to be independent

* One defines $X_t$ for $t \in [0,1]$ as solution of the ODE with drift function $\mu(t,x,\red{c})$:

$$
dX_t = \mu(t, X_t, \red{C}) \, dt
$$

* **Requirement:** The marginal distribution of $X_1$ is $\green{\pi_1(dx)}$

$$
\green{X_1 \sim \pi_1(dx)}
$$

---

* $(X_0, C)$ where $\red{C = (X_0, X_1)}$ and $X_0 \perp X_1$

$$
\begin{align}
\; & \mu(x, x_0, x_1) = x_1-x_0\\
\; &  dX_t = \red{(X_1 - X_0)} \, dt\\
\; & X_t = (1-t) \, X_0 + t \, X_1
\end{align}
$$
...
::: {style="text-align:center;"}
![](./data/straightflow.gif){width=800}
:::

---

* $(X_0, C)$ where $\red{C = (X_0, X_1, Z)}$ and $X_0 \perp X_1$ and $Z \sim \normal(0,1) \in \bbR^{D}$. For a function $\gamma:[0,1] \to \bbR$ such that $\gamma(0) = \gamma(1) = 0$:

$$
\begin{align}
\; & \mu(x, x_0, x_1, z) = (x_1-x_0) + \gamma'(t) \, Z\\
\; & dX_t = \red{(X_1 - X_0 + \gamma'(t) \, Z)} \, dt\\
\; & X_t = (1-t) \, X_0 + t \, X_1 + \gamma(t) \, Z
\end{align}
$$

::: {style="text-align:center;"}
![](./data/straightflow_noisy.gif){width=800}
:::
...
* $(X_0, C)$ where $\red{C = (X_0, X_1, Z)}$ with:
   * $X_0 \sim \blue{\pi_0(dx)}$ and $X_1 \sim \green{\pi_1(dx)}$
   * $Z \sim \normal(0,1) \in \bbR^{D}$
   * Do not assume any independence between $X_0$, $X_1$
* For some function $\alpha(\cdot)$ and $\beta(\cdot)$ and $\gamma$ such that

$$
\begin{align}
\alpha(0) &= 1 \qquad \textrm{and} \qquad \alpha(1) = 0\\
\beta(0) &= 0 \qquad \textrm{and} \qquad \beta(1) = 1
\end{align}
$$

* Define the quantity $X_t$ as $\blue{X_t = \alpha(t) \, X_0 + \beta(t) \, X_1 + \gamma(t) \, Z}$

* This is implicitly described by the ODE

$$
dX_t = \red{ \curBK{\alpha'(t) \, X_0 + \beta'(t) \, X_1 + \gamma'(t) \, Z} } \, dt
$$

---

* Let $p_t(dx) = \P(X_t \in dx)$ be the marginal distribution of $X_t$ for $t \in [0,1]$
* Interpolation between $\blue{\pi_0(dx)}$ and $\green{\pi_1(dx)}$:

$$
\blue{p_0(dx) \equiv \pi_0(dx)} \qquad \textrm{and} \qquad \green{p_1(dx) \equiv \pi_1(dx)}
$$

* Not useful as defined since the conditionning variable $C$ depends on $\pi_1(dx)$

* <span style='color:blue'>**Objective:**</span> derive a drift function $\red{\bar{\mu}(t,x)}$ that does **not** depend on $C$ and such that the following ODE has the same marginals

$$
\red{d \bar{X}_t = \bar{\mu}(t, \bar{X}_t) \, dt}
$$

* For all time $0 \leq t \leq T$ we have

$$
\red{\P(\bar{X}_t \in dx) \; = \;  p_t(dx)} \; = \; \P(X_t \in dx)
$$


---

* First order Taylor expansion for test function $\phi$:

$$
\begin{align}
\E_{\pi_{t+\delta}}(\phi) &=
\E[\phi(X_{t+\delta})] \\
&\approx \E[\phi(X_t) \; + \; \phi'(X_t) \, \blue{\mu(t, X_t, C)} \, \delta]\\
&= \E[\phi(X_t) \; + \; \phi'(X_t) \, \red{\bar{\mu}(t, X_t)} \, \delta]
\end{align}
$$

* This shows that $p_t$ satifies the Fokker-Planck equation:

$$
\partial_t p_t \; = \; -\nabla \cdot \curBK{ \red{\bar{\mu}(t,x)} \, p_t} 
$$

* The drift function $\red{\bar{\mu}(t,x)}$ is defined as a <span style='color:blue'>**conditional expectation**</span>:

$$
\red{\bar{\mu}(t,x) \; = \; \E[\mu(t, x, C) \mid X_t = x]}
$$
...
* Usual approach: parametrize with a neural net and minimize

$$
\cL(\theta) \equiv \E \curBK{ \int_{t=0}^T \, \lambda(t) \, 
\left\| \blue{\mu(t, X_t, C)} \; - \; \red{\bar{\mu}(t,X_t)} \right\|^2 \, dt}
$$

* The method is <span style='color:blue'>**simulation-free**</span> if one can directly sample $X_t$:
   * Example 1: $X_t = (1-t) \, X_0 + t \, X_1$
   * Example 2: $X_t = (1-t) \, X_0 + t \, X_1 + \sigma(t) \, Z$
...
* We have $x = (1-t) \, \widehat{x}_0(t,x) + t \, \widehat{x}_1(t,x)$ so that

$$
\begin{align}
\red{\bar{\mu}(t,x)}
&= \E[X_1 - X_0 \mid X_t = x] \\
&= \widehat{x}_1(t,x) - \widehat{x}_0(t,x)\\
&= - \frac{x - \widehat{x}_1(t,x)}{1-t}
\end{align}
$$

* Generate data with by sampling $\bar{X}_0 \sim \pi_0(dx)$ and follow the flow:

$$
d \bar{X}_t = - \frac{\bar{X}_t - \widehat{x}_1(t,\bar{X}_t)}{1-t} \, dt
$$
...
* Interpolation: $\blue{X_t = \alpha(t) \, X_0 + \beta(t) \, X_1 + \gamma(t) \, Z}$

* ODE: $dX_t = \curBK{\alpha'(t) \, X_0 + \beta'(t) \, X_1 + \gamma'(t) \, Z}  \, dt$

* Flow matching:

$$
d \bar{X}_t = \underbrace{
\curBK{
\alpha'(t) \, \red{\widehat{x}_0(t,\bar{X}_t)} \; + \; 
\beta'(t) \, \red{\widehat{x}_1(t,\bar{X}_t)} \; + \;
\gamma'(t) \, \red{\widehat{z}(t,\bar{X}_t)}}}_{\red{\bar{\mu}(t,\bar{X}_t)}}  \, dt$$
...
* The marginal at time $0 \leq t \leq 1$ reads:

$$
p_t(dx) \; = \; \frac{1}{\cZ_t} \int \exp\curBK{-\frac{(x - [\alpha(t) x_0 + \beta(t)])^2}{\gamma(t)^2}} \, \pi(dx_0, dx_1)
$$

* Algebra gives:

$$
\blue{\cS(t,x) \; = \; \nabla \log p_t(x) \; = \; -\frac{\widehat{z}(t,x)}{\gamma(t)}}
$$

* <span style='color:blue'>SDE-ODE trick:</span> for volatility $\sigma > 0$ the following SDE shares the same marginals as the ODE flow

$$
d \bar{X}_t = \bar{\mu}(t, \bar{X}_t) \, + \, \frac{\sigma^2}{2} \blue{\cS(t,\bar{X}_t) }  dt \; + \; \green{\sigma \, dW}
$$

---

::: {style="text-align:center;"}
![](./data/flow_checkboard.mp4){width=800}
:::

---

::: {style="text-align:center;"}
![](./data/flow_checkboard_exact_trajectories.png){width=800}
:::

---

* Consider the case $(X_0, C)$ where $C=(X_0, X_1)$
* Many applications when it is useful to **not** choose $X_0$ and $X_1$ independent
   * Image denoising: $X_0$ is the noisy image and $X_1$ is the clean image
    * Image inpainting: $X_0$ is the incomplete image and $X_1$ is the complete image

::: {style="text-align:center;"}
![](./data/flow_inpainting.jpg){width=600}

Paper: ["Stochastic Interpolants with data-dependent couplings"](https://arxiv.org/abs/2310.03725)
:::

---

### Flow matching for SDEs

* One defines $X_t$ for $t \in [0,1]$ as solution of the SDE

$$
dX_t = \mu(t, X_t, \red{C}) \, dt + \sigma(t, \red{C}) \, dW
$$
...
* Same proof gives that the SDE has the same marginals

$$
d \bar{X}_t = \red{\bar{\mu}(t, \bar{X}_t)} \, dt + \red{\bar{\sigma}(t)} \, dW
$$

$$
\red{\bar{\mu}(t,x) = \E[\mu(t, x, C) \mid X_t = x]}
\qquad \textrm{and} \qquad
\red{\bar{\sigma}^2(t) = \E[\sigma^2(t, C) \mid X_t = x]}
$$

---

* A Brownian Bridge with target $x_\star$ is a Brownian motion conditioned to reach $x_\star$ at time $T=1$. It can be described as

$$
dX_t = -\frac{(X_t - x_\star)}{1-t} \, dt \; + \; dW
$$

* Consider: $(X_0, C)$ with $\red{C=X_1}$ where $X_0 \perp X_1$ and dynamics

$$
\begin{align}
\; & \mu(t, x, x_1) = -\frac{(x - x_1)}{1-t}\\
\; & \sigma(t, x_1) = 1\\
\; & dX_t = \red{-\frac{(X_t - X_1)}{1-t}} \, dt \; + \; dW
\end{align}
$$

* The resulting flow-matched SDE reads

$$
d \bar{X}_t = -\frac{(\bar{X}_t - \red{\widehat{x}_1(t,\bar{X}_t)})}{1-t} \, dt \; + \; dW
$$
...
* Maximum A Posteriori: $x_{\map} \; = \; \argmax_{x} \pi_{\post}(x)$

* Markov Chain Monte Carlo: a Markov process (eg. Langevin) explores $\pi_{\post}(dx)$

* Variational Methods: $\argmin_{\lambda} \; \kl\BK{p_{\lambda}, \pi_{\post}}$
...
\underbrace{
\blue{ \nabla \log \P(Y \, | x_t) }
}_{\text{(difficult to compute)}}
$$

---

How to obtain the term: $\blue{ \nabla \log \P(Y \, | x_t) }$

* Naive approximation: $\P( x_0 \, | x_t) \; \approx \; \delta(\widehat{x}_0(t, x_t))$
...
& \approx \P(\, Y | \widehat{x}_0(t, x_t) \,)
\end{align}
$$

* Better approximation: $\P( x_0 \, | x_t) \; \approx \; \blue{\widehat{\P( x_0 \, | x_t)}}$
$$
\P(Y \, | x_t) 
=
\int \P(Y | x_0) \, \P( x_0 \, | x_t) \, dx_0 \\
\approx 
\int \P(Y | x_0) \, \blue{ \widehat{\P( x_0 \, | x_t)} } \, dx_0 
$$
...
* Consider a large dataset of labeled samples $\{x_i, y_i\}_{i=1}^{N}$. Objective is to create new samples **conditioned** on a given label $y_\star$
* One can train a standard diffusion model on $\{x_i\}_{i=1}^N$

$$
dX_t = \frac12 \, X_t \, dt \; + \; \nabla \log p_t(X_t) \, dt \; + \; dB
$$

* To generate samples conditioned on a given label $y_\star$

$$
dX_t = \frac12 \, X_t \, dt \; + \; \nabla \log p_t(X_t) \, dt \; + \,  \blue{\nabla \log \P(y_\star \mid X_t)}\; + \; \; dB
$$

* Train a <span style='color:blue'>new classifier</span> to estimate the term $\blue{\nabla \log \P(y_\star \mid X_t)}$

---

::: {style="text-align:center;"}
![](./data/conditional_generation.jpg){width=800}
...
* Consider a large dataset of labeled samples $\{x_i, y_i\}_{i=1}^{N}$.
* Instead of training a generative model as well as an additional classifier, one can directly train a generative model on the labeled dataset, i.e. <span style='color:blue'>**no classifier**</span>

* Standard score estimate with denoising diffusion:

$$
\nabla \log p_t(x) = -\frac{x - \alpha_t \, \widehat{x}_0(t,x)}{1-\alpha_t^2}
$$

* Same with labeled data: estimate $X_0$ given $X_t$ and the associated label $y$:

$$
\nabla \log p_t(x \mid \red{y}) = -\frac{x - \alpha_t \, \widehat{x}_0(t,x, \red{y})}{1-\alpha_t^2}
$$

---

* Text embedding + Classifier-free guidance + Tricks

::: {style="text-align:center;"}
![](./data/text-to-img.jpg){width=800}

[Paper (2022): "Photorealistic Text-to-Image Diffusion Models with Deep Language Understanding"](https://arxiv.org/abs/2205.11487)
:::

---

::: {style="text-align:center;"}
![](./data/infill.gif){width=600}
