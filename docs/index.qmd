...
* Instead, emphasis on intuition and heuristics
...
* For a test function $\phi: \bbR^d \to \bbR$
...
* The density evolves according to the transport/continuity PDE:
$$
\partial_t \pi_t \; = \; - \nabla \cdot (\mu \, \pi_t)
$$
...
* Given a large set of samples $\{ x_i \}_{i=1}^N \subset \bbR^D$, <span style='color:blue'>generate</span> "similar looking" samples?
...
* Distribution $q_\lambda$ is obtained by applying a differentiable transformation $T_\lambda$ to a simple reference distribution $\pi_{\text{ref}}$. The reference distribution is often chosen as a standard Gaussian distribution $\pi_{\text{ref}} = \normal(0,I)$.

* Generating $\blue{X \sim q_\lambda}(dx)$ is equivalent to generating $\green{Z \sim \pi_{\text{ref}}(dz)}$ and applying $T_\lambda$:

$$
\blue{X} = T_\lambda(\green{Z})
$$

* Change of variables formula gives the density of $q_\lambda(dx)$:
$$
q_\lambda(\blue{x}) = \pi_{\text{ref}}(\green{z}) \, \times \, \left| \text{Jac}(T_\lambda)(\green{z}) \right|^{-1}
$$
where $\blue{x} = T_\lambda(\green{z})$.
...
* Animation below by [Eric Jang](https://blog.evjang.com/2019/07/likelihood-model-tips.html)

![](./data/normalizing_flow.gif){.absolute bottom=0% left=25% width="50%"}

---

#### Fitting a normalizing flow model
$$
\blue{ q_\lambda(x) = \pi_{\text{ref}}(z) \, \times \, \left| \text{Jac}(T_\lambda)(z) \right|^{-1} }
$$

* Can be fitted to data $\{x_i\}_{i=1}^N$ by MLE:
...
* **Questions:**
  * <span style='color:blue'>How to design $T_\lambda$ so that the family of distributions $\{ q_\lambda \}_{\lambda \in \Lambda}$ is expressive?</span>
  * How to make sure that $T_\lambda$ is invertible?
  * How to make sure that the inverse of $T_\lambda$ is easy to compute?
  * How to make sure that the Jacobian is easy to compute?
...
* Instead, try to design an appropriate drift function $\mu(t,x)$ so that the diffusion
...
* active area of research with a [many of recent developments](https://github.com/diff-usion/Awesome-Diffusion-Models)
* this course: emphasis on the big picture and heuristic derivations  
...
* **Things not discussed**:
  ...
  * advanced discretization of the diffusion processes

  ![](./data/denoising_diffusion_trend.jpg){.absolute bottom=0% left=33% width="40%"}
...
![](./data/conditional_VIASM.jpg){.absolute bottom=0% left=25% width="50%"}
...
* Obtain the Fokker-Planck equation $\blue{ \widetilde{p}_t(x) \equiv p_{T-t}(x) }$
...
* Original [proof](https://doi.org/10.1016/0304-4149(82)90051-5) by Brian Anderson in 1982
